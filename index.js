...
lines.forEach(function(RLline) {
	...
	let commandHandler = {
		print: () => {
			if (commandArgs[1]) {
				fs.appendFileSync(compiledFile, "\n" + ("\t")
					.repeat(tab) + "console.log(`" + commandArgs.slice(1)
					.join(" ")
					.replace(/\$(.+?)\$/g, '${$1}') + "`);")
			} else {
				fs.appendFileSync(compiledFile, "\n" + ("\t")
		...
		let: () => {
			if (commandArgs.length > 2) {
				...
				fs.appendFileSync(compiledFile, "\n" + ("\t")
					.repeat(tab) + "let " + commandArgs[2] + " = " + prefix + commandArgs.slice(3)
					.join(" ")
					.replace(/\$(.+?)\$/g, '${$1}') + suffix + ";");
			} else {
				fs.appendFileSync(compiledFile, "\n" + ("\t")
		...
		if: () => {
			if (commandArgs.slice(1)
				.join(" ")
				.replace(/\$(.+?)\$/g, '${$1}')) {
				fs.appendFileSync(compiledFile, "\n" + ("\t")
					.repeat(tab) + "if (" + commandArgs.slice(1)
					.join(" ") + ") {")
				tab++
			} else {
				fs.appendFileSync(compiledFile, "\n" + ("\t")
		...
		else_if: () => {
			if (commandArgs.slice(1)
				...
				tab--
				fs.appendFileSync(compiledFile, "\n" + ("\t")
					.repeat(tab) + "} else if (" + commandArgs.slice(1)
					.join(" ") + ") {")
				tab++
			} else {
				fs.appendFileSync(compiledFile, "\n" + ("\t")
		...
		ask: () => {
			if (commandArgs[1]) {
				fs.appendFileSync(compiledFile, "\n" + ("\t")
					.repeat(tab) + "output = prompt()(`" + ("" || commandArgs.slice(1)
						.join(" "))
					.replace(/\$(.+?)\$/g, '${$1}') + "`);")
			} else {
				fs.appendFileSync(compiledFile, "\n" + ("\t")
		...
		func: () => {
			if (commandArgs[1] && commandArgs[2]) {
				fs.appendFileSync(compiledFile, "\n" + ("\t")
					.repeat(tab) + "function " + commandArgs[1] + "(" + commandArgs.slice(2)
					.join(",") + "){")
				tab++
			} else {
				fs.appendFileSync(compiledFile, "\n" + ("\t")
		...
		call: () => {
			if (commandArgs[1]) {
				let funcArg = commandArgs.slice(2)
					.join(" ")
				fs.appendFileSync(compiledFile, "\n" + ("\t")
		...
		"=": () => {
			if (commandArgs[1] && commandArgs.slice(2)
				.join(" ")
				.replace(/\$(.+?)\$/g, '${$1}')) {
				fs.appendFileSync(compiledFile, "\n" + ("\t")
					.repeat(tab) + commandArgs[1] + " = `" + commandArgs.slice(2)
					.join(" ")
					.replace(/\$(.+?)\$/g, '${$1}') + "`")
			} else {
				fs.appendFileSync(compiledFile, "\n" + ("\t")
		...
		"custom": () => {
			if (commandArgs[1]) {
				fs.appendFileSync(compiledFile, "\n" + ("\t")
					.repeat(tab) + commandArgs.slice(1)
					.join(" ")
					.replace(/\$(.+?)\$/g, '${$1}'))
			} else {
				fs.appendFileSync(compiledFile, "\n" + ("\t")
		...
		"readfile": () => {
			if (commandArgs[1]) {
				fs.appendFileSync(compiledFile, "\n" + ("\t")
					.repeat(tab) + "if (fs.existsSync(`" + commandArgs.slice(1)
					.join(" ")
					.replace(/\$(.+?)\$/g, '${$1}') + "`)) {")
				tab += 1
				fs.appendFileSync(compiledFile, "\n" + ("\t")
					.repeat(tab) + "output = fs.readFileSync(`" + commandArgs.slice(1)
					.join(" ")
					.replace(/\$(.+?)\$/g, '${$1}') + "`, 'utf8');")
				tab -= 1
				fs.appendFileSync(compiledFile, "\n" + ("\t")
		...
		"writefile": () => {
			if (commandArgs[1]) {
				fs.appendFileSync(compiledFile, "\n" + ("\t")
					.repeat(tab) + "fs.writeFileSync(`" + commandArgs[1].replace(/\$(.+?)\$/g, '${$1}') + "`,`" + commandArgs.slice(2)
					.join(" ")
					.replace(/\$(.+?)\$/g, '${$1}') + "`);")
			} else {
				fs.appendFileSync(compiledFile, "\n" + ("\t")
		...
		"import": () => {
			if (!fs.existsSync("./libs/" + commandArgs.slice(1)
					.join(" ") + ".js")) {
				fs.appendFileSync(compiledFile, "\n" + ("\t")
					.repeat(tab) + "// Error processing the line " + lineNum)
				errors.push(`Argument 1 missing in line ${lineNum}`)
			}
			imports.push(commandArgs.slice(1)
				.join(" "))
		},
		"": () => {},
	}
